defmodule CircuitsV2.I2CNifTest do
  use ExUnit.Case
  ...
  test "unloading NIF" do
    ...
    # few times.
    for _times <- 1..10 do
      assert {:module, CircuitsV2.I2C.Nif} == :code.ensure_loaded(CircuitsV2.I2C.Nif)
      ...
      assert true == :code.delete(CircuitsV2.I2C.Nif)

      # The purge will call the unload which can be verified by turning DEBUG on
      # in the C code.
      assert false == :code.purge(CircuitsV2.I2C.Nif)
    end
  end

  test "setting backend to unknown value doesn't load the NIF" do
    original_backend = Application.get_env(:circuits_v2_i2c, :default_backend)

    # Unload the current code if loaded
    _ = :code.delete(CircuitsV2.I2C.Nif)
    _ = :code.purge(CircuitsV2.I2C.Nif)

    # Attempt loading. NIF shouldn't be loaded this time.
    Application.put_env(:circuits_v2_i2c, :default_backend, Some.Other.Backend)
    assert {:module, CircuitsV2.I2C.Nif} == :code.ensure_loaded(CircuitsV2.I2C.Nif)
    assert_raise UndefinedFunctionError, fn -> CircuitsV2.I2C.info() end

    # Cleanup
    assert true == :code.delete(CircuitsV2.I2C.Nif)
    assert false == :code.purge(CircuitsV2.I2C.Nif)
    Application.put_env(:circuits_v2_i2c, :default_backend, original_backend)
  end
